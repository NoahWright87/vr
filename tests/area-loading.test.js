...
test('destinations are lazy fragments instead of simultaneous scene entities', () => {
  ...
  assert.match(dayNight, /count: \{ type: 'int', default: 36 \}/);
  assert.match(dayNight, /cellCount: \{ type: 'int', default: 5 \}/);
  ...
  assert.match(dayNight, /cloud\.shadowProxy\.position\.set/);
  assert.match(dayNight, /candidates\.slice\(0, this\.data\.shadowCasters\)/);
