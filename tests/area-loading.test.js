...
test('destinations are lazy fragments instead of simultaneous scene entities', () => {
  ...
  assert.match(dayNight, /verticalSpread: \{ type: 'number', default: 18 \}/);
  assert.match(dayNight, /orientationVariation: \{ type: 'number', default: 12 \}/);
  ...
  assert.match(dayNight, /new THREE\.PlaneGeometry\(1, 1, 4, 4\)/);
  assert.match(dayNight, /side: THREE\.FrontSide/);
  assert.match(dayNight, /uv\.setXY\(uvIndex/);
  assert.match(dayNight, /this\.cloudOrientation\.makeBasis/);
  assert.match(dayNight, /dummy\.quaternion\.setFromRotationMatrix\(this\.cloudOrientation\)/);
  assert.match(dayNight, /rotation: this\.randomBetween\(-orientationVariation, orientationVariation\)/);
  assert.match(dayNight, /sizeCeiling = this\.data\.minSize/);
  assert.match(dayNight, /var isWispy = Math\.random\(\) < 0\.14/);
