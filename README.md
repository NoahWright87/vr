...
- **[Menus](primitives/menus/index.html)** — every menu style in one scene, on a checkerboard floor (a tiny canvas-generated texture, tiled) so there's some sense of scale and footing:
  ...
  - **Bare-hand tracking** — every interaction above also works with the Touch controllers set down, using the real WebXR Hand Input API instead (`common/hand-tracking.js`). A dedicated, purely-decorative entity per hand carries A-Frame's own `hand-tracking-controls` (a real low-poly rigged hand mesh following your actual finger joints); `hand-tracking-follow` reads its tracked wrist pose every frame and feeds it into the exact same `semantic-hand.setWorldTransform` desktop's simulated hand already uses, so the watch, hint-zone reach, and the fingertip laser all work unmodified regardless of what's actually moving the hand. A real Touch controller has two independent buttons — grip (arms a hand for grabbing/pointing) and trigger (clicks whatever it's pointing at) — that hand tracking's one gesture, a pinch, stands in for both at once: pinch near a box to grab or hold it (release the pinch to drop it), or point at a menu row and pinch to click it. The one wrinkle: firing a synthetic click in the same instant as first enabling the fingertip raycaster would see no cached hover target yet (nothing's had a chance to hit anything), so `hand-tracking-follow` forces one fresh raycast right before dispatching the click — the same trick A-Frame's own `cursor` component uses internally for mouse-touchstart and `xrselect`.
