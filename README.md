...
- **[Menus](primitives/menus/index.html)** — every menu style in one scene, on a checkerboard floor (a tiny canvas-generated texture, tiled) so there's some sense of scale and footing:
  ...
  - **Bare-hand tracking (finger gun)** — every interaction above also works with the Touch controllers set down, using the real WebXR Hand Input API instead. `finger-gun-controls` (`common/hand-tracking.js`) is a bare-hand input *source*, the hand-tracking equivalent of `oculus-touch-controls`: point your index finger and cock your thumb up like a hammer to aim — mirrors squeezing a real grip button to point — then drop the thumb until it's roughly parallel with the finger to fire — mirrors pulling a real trigger. It does its own direct `frame.getJointPose` polling (the stable WebXR joint names, not A-Frame's own `hand-tracking-controls` internals) for exactly the six joints the gesture needs, classifies the pose with a small pure, unit-tested gesture module (`common/hand-gestures.js`, `tests/hand-gestures.test.js`), and then speaks the same small action vocabulary every other input already speaks: `gripdown`/`gripup` when the gun cocks or relaxes, `triggerdown`/`triggerup` when the hammer drops or resets. Nothing downstream — `semantic-hand`'s grab/point handling, the watch's fingertip cursor, hint-zone reach — needed to change or even know hand tracking exists, the same way none of it cares whether a keyboard, a gamepad, or a real Touch controller produced those events; that's the point of speaking the shared vocabulary rather than inventing a parallel one. It also drives the hand's position (feeding the real tracked wrist pose into `semantic-hand.setWorldTransform`, the same method desktop's simulated hand already uses) and publishes the real per-frame fingertip position and aim direction, which `hand-with-watch` uses to override the fingertip laser's usual fixed-offset approximation with the real thing while a finger gun is actively pointing. A separate, purely decorative entity still renders a real hand mesh via A-Frame's own `hand-tracking-controls`, entirely decoupled from `finger-gun-controls`' own tracking.
...
- **[Hand Tracking](primitives/hand-tracking/index.html)** — the smallest possible bare-hand demo: put the Touch controllers down, enter VR, and pinch near the floating box to pick it up; un-pinch to let go. Built entirely from A-Frame's own `hand-tracking-grab-controls` (no custom grab code) plus one small `settle-to-floor` component so a dropped box actually falls to the floor instead of freezing wherever you let go — `hand-tracking-grab-controls` has no concept of gravity on its own. Headset-only; there's no meaningful mouse/keyboard equivalent for bare-hand input, so unlike Cube Pop there's no desktop/gaze fallback here.
