...
- **[Menus](primitives/menus/index.html)** — every menu style in one scene, on a checkerboard floor (a tiny canvas-generated texture, tiled) so there's some sense of scale and footing:
  ...
  - **Bare-hand tracking (canonical gestures)** — every interaction above also works with the Touch controllers set down, using the real WebXR Hand Input API instead. `hand-gesture-controls` (`common/hand-tracking.js`) is a bare-hand input *source*, the hand-tracking equivalent of `oculus-touch-controls`: it does its own direct `frame.getJointPose` polling (the stable WebXR joint names, not A-Frame's own `hand-tracking-controls` internals) for the handful of joints needed to read finger curl and thumb position, and classifies the tracked hand's shape every frame into one of a small, named catalog of canonical gestures — a hand-tracking equivalent of a gamepad's fixed button layout — defined as pure, unit-tested logic in `common/hand-gestures.js` (`tests/hand-gestures.test.js`), independent of AFRAME/WebXR so it's reusable by a differently-sourced hand tracker later: **pinch** (thumb and index touching), **okay 👌** (thumb and index touching, other fingers extended), **fist ✊** (every finger, including the thumb, curled — a real grabbing motion), **finger gun** (index extended, other fingers curled — point your index finger and cock your thumb up like a hammer to aim), **finger gun fired** (the thumb dropped to roughly parallel with the index finger — the "hammer falling"), and **thumbs up/down** (a fist with the thumb extended and pointing up or down instead of curled in). Some of these drive the same small action vocabulary every other input already speaks — `gripdown`/`gripup` (a pinch, a fist, or a cocked/fired finger gun — mirrors squeezing a real grip button; a pinch and a fist are deliberately both wired to the same grab, since either reads as "grab" and neither is more correct than the other) and `triggerdown`/`triggerup` (a fired finger gun only — mirrors pulling a real trigger) — so nothing downstream (`semantic-hand`'s grab/point handling, the watch's fingertip cursor, hint-zone reach) needed to change or even know hand tracking exists, the same way none of it cares whether a keyboard, a gamepad, or a real Touch controller produced those events. The rest (okay, thumbs up/down) are recognized and broadcast — as a generic `gesture-changed` event any future experience can listen for — but not wired to anything yet; that's the point of the catalog being separate from what a given experience maps it onto. `hand-gesture-controls` also drives the hand's position (feeding the real tracked wrist pose into `semantic-hand.setWorldTransform`, the same method desktop's simulated hand already uses) and publishes the real per-frame fingertip position and aim direction, which `hand-with-watch` uses to override the fingertip laser's usual fixed-offset approximation with the real thing while a finger gun is actively pointing. A separate, purely decorative entity still renders a real hand mesh via A-Frame's own `hand-tracking-controls`, entirely decoupled from `hand-gesture-controls`' own tracking.

  Feeding a raw WebXR wrist pose straight into `semantic-hand` doesn't work unmodified, the same problem the watch's controller-based positioning ran into originally: the WebXR Hand Input spec's own joint orientation convention (-Z local = distal, along the bone toward the fingers; -Y local = dorsal, outward from the palm) has nothing to do with a Touch controller's grip-pose convention, which is what the watch's band/face geometry is actually built against — fed unmodified, the band circles the wrong axis entirely and the face points out to the side instead of off the back of the hand. `hand-gesture-controls` corrects for this with one fixed, per-hand quaternion (`_wristCorrection`, derived and checked numerically against both hands across several poses — band-to-forearm and face-to-dorsal alignment both land within floating-point error of exactly 0°) before ever calling `setWorldTransform`, so a WebXR-tracked wrist produces a pose indistinguishable, as far as `hand-with-watch`/`semantic-hand` are concerned, from a real Touch controller's.

  An optional `gesture-hud` component shows which canonical gesture is currently recognized — not as a label floating in the 3D scene near the hand (tried first; reads as far more obtrusive than it sounds, since it competes with everything else at world scale and follows the hand into your peripheral vision), but as a fixed slot in a Halo-style visor HUD: a plain child of the camera at a constant local offset, the same "attach straight to `<a-camera>`, no extra rotation needed" convention Cube Pop's own HUD text already uses, so it simply never moves on screen regardless of where the source entity actually is. See the Hand Tracking primitive below for how its three slots (left hand, right hand, and arm-swing's own whole-body announcements) are laid out, on by default with a watch-menu toggle to turn them off.
...
- **[Hand Tracking](primitives/hand-tracking/index.html)** — a bare-hand demo built on the exact same shared hand rig as the Menus showcase (`hand-with-watch` + `semantic-hand` + `hand-gesture-controls`, see "Bare-hand tracking" above), not a standalone one-off: each wrist gets a real watch, correctly oriented (see above), a pinch or a fist grabs and holds the floating box (the shared `simple-grabbable`/`hint-zone` primitive, same as the showcase's own test box — release either gesture to drop it, and it falls to the floor on its own), and pointing a finger gun at either watch opens it, with a drop of the thumb clicking whatever it's pointing at. Since this is explicitly meant as a demo of the gesture catalog itself, `gesture-hud` is on by default in all three of its slots — left hand's recognized gesture bottom-left, right hand's bottom-right, and arm-swing's own brief "Step 👟" announcement (see below) bottom-center, all fixed to the visor rather than floating over the hand — with a "Labels" row in the watch menu to turn them off once they're more distracting than useful. The idea: this page is where new canonical gestures get demonstrated and labeled as they're added to the catalog, so a future game can see exactly what's available to map onto its own controls. Headset-only; there's no meaningful mouse/keyboard equivalent for bare-hand input, so unlike Cube Pop there's no desktop/gaze fallback here.

  It's also where **arm-swing locomotion** lives — an experiment, not a validated locomotion scheme: swing your arms like you're actually walking and each swing takes a step, sliding the rig forward (in whatever direction the camera is facing) by `stepDistance`. The whole thing turns on one design choice, `updateArmSwingState` in `common/arm-swing.js`: a step only fires when one arm swings from armed ("back", past `armThreshold` behind an approximated shoulder position) to forward, **and the other arm is confirmed to currently be back at that same instant** — real walking swings the two arms in opposite phase, and almost nothing else people do with their hands does (reaching, pointing, and gesturing all move one arm alone or both arms together), so requiring that bilateral alternation is what keeps this from firing while doing literally anything else in the scene. It's a different shape of building block than the instant-in-time pose classification in `hand-gestures.js` — a swing is a motion *pattern over time*, so it carries small per-arm `{ armed }` state across frames instead of being a stateless classification, tested the same way (`tests/arm-swing.test.js`) regardless. `arm-swing-locomotion` (`common/arm-swing-locomotion.js`) is the component wiring it up: it reads `leftHand`/`rightHand`'s own world position every frame, so — like everything else in this catalog — it doesn't care what's actually moving them (Touch controllers work too, not just hand tracking). Each step also emits the same `gesture-changed` event `hand-gesture-controls` does (a brief "Step 👟", self-clearing after `stepAnnounceMs`), so the shared HUD (see above) can show this whole-body motion in its own bottom-center slot the same way it shows a per-hand pose, without either one knowing about the other. A "Walk" row in the watch menu turns it off if it gets in the way of testing anything else.
