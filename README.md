...
**Pause menu:** squeeze either controller's grip button (or the "Menu" button in the desktop overlay) to open it. Opening it disables punching and cube-popping entirely and freezes rig physics (both gated on a shared `menuOpen` flag), and switches on a laser pointer + cursor on each hand — same raycaster+cursor pattern `laser-controls` itself uses, just aimed at menu buttons instead of cubes and normally left off. Aim at a button and pull the trigger to click it; the trigger's normal "quick reset" job is suspended while the menu is open so a click there doesn't also reset the arena. From the menu you can adjust **Move Speed** (a single multiplier over the whole punch-to-move impulse, since "does punching feel strong enough" is the real question, not any one internal constant), **Cube Count**, cycle **Cube Behavior**, **Reset Arena**, **Resume**, or — the direct fix for getting stuck in VR — **Exit VR**. (An earlier revision used gaze-and-dwell on the head reticle instead of hand lasers — dropped because it wasn't usable in practice; the reticle still works as the desktop/phone fallback pointer for the menu, same as it does for cubes.)
...
5. Squeeze either grip to open the pause menu — this gives you a laser pointer on each hand and disables punching. Aim at a button and pull the trigger to click it: adjust speed/cube count/behavior, Resume, or select **Exit VR** to leave the session.
