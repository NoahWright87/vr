...
- **[Menus](primitives/menus/index.html)** — every menu style in one scene, on a checkerboard floor (a tiny canvas-generated texture, tiled) so there's some sense of scale and footing:
  ...
  - **Crossbar menu** — the new shared menu system, and the one to build on. A page is *data* (a title and a list of items, `common/menu-model.js`), a surface says how many rows it can show, and the list windows to fit — so twelve destinations sit in a five-row panel and a thirteenth changes no markup. Everything a player can change is a submenu: a value row opens into its choices, scrolling previews live, and backing out commits (toggles are the exception and flip in place). Continuous numbers render like an endless list without being one — the level computes the handful of values around the focus from min/max/step, so a 0–20 knob at 0.1 steps never builds 201 rows. Navigation is six verbs, so a thumbstick, a finger laser, a poke and the keyboard all drive the same rows: up/down moves, inward enters, outward commits and comes back. Changing level animates: the new title's letters fly out of the row you picked and settle into the breadcrumb rail one at a time, and fly back into the list when you leave — so going deeper reads as the item *becoming* the heading. Closing a world panel collapses it to its title bar rather than deleting it from the room, since a panel standing on its own has no trigger to bring it back (a watch or visor menu, which does, still hides outright). Pressing outward at the top level lands on the panel's own **close button** rather than dismissing it — closing is a thing you confirm, not something one press too many can do by accident, and it puts the title bar's controls inside the focus ring where a stick can reach them. A fixture sets `closable: false` instead and gets no X at all, with back-at-root doing nothing: a wall panel or the visor is not something you should be able to lose, and the visor already closes by taking your hand off your head. Panels that do collapse stay reachable in a headset too — reaching for a collapsed one opens it again, since in XR there is no walk-up `E` prompt to bring it back. Tapping the focused row selects it; tapping any other row only scrolls to it, so a tap is never dangerous. **Confirming always flashes**, whatever the row does: the plate under it snaps to full, its label inverts, and both ease back over ~240 ms. Most rows leave something visibly different behind anyway, but a plain action — `Reset boxes` — spends its whole effect somewhere else in the game, and without the flash a press that worked and a press that was dropped look identical. Since the flash is painted before the row acts and survives the render the row triggers, drilling into a category flashes it on the way in, while its letters are still flying out to the breadcrumb rail. Off a headset you **enter** a menu the same way you would in VR: walk up until its `E Menu` prompt appears (a standard `hint-zone`, so it competes with every other prompt in the room rather than shouting over them), press `E`, and that panel takes the controls — it brightens, spells out its keys along the bottom, and W/S/A/D drive it and nothing else. Walking is suspended while you hold a menu, and `E` or `Esc` hands it back with the menu left open where it was. A second panel in the same room never moves, because exactly one menu can be entered at a time. Pointing still works throughout: the gaze cursor clicks rows whether or not you have entered anything. In XR there is no prompt to press — **stick control is opt-in by posture:** hold an empty hand near a panel — not pointing, the finger laser takes precedence, and not holding anything — and the panel *and that hand* light up to show which stick just changed meaning. Drop your arm and the stick is yours again immediately while the menu stays where it was. Only the nearer hand can claim a menu, and only one hand at a time. The model has no A-Frame in it and is covered by `tests/menu-model.test.js`.
  - **Visor menu** (`common/visor-menu.js`) — the third surface, and the same `crossbar-menu` with four values turned on: a curve, inboard text alignment, a one-sided scrim, and optionally a single eye. Head-locked at 1.8 m, anchored so its inboard edge sits ~15° off centre and its outboard edge ~32°, which keeps the middle of your view clear. **How it is drawn is a live setting** — the `Draw` row inside the visor itself, so the two can be compared in a headset rather than argued about:

  - `In world (stereo)`, the default (`overlay: 'world'`). The panel is a real object at 1.8 m that takes no part in the depth test and draws after everything else, so a wall or doorway nearer than it can't cut through it — but both eyes still converge where it actually is. Turning the depth test off means draw order alone decides what covers what, which the panel's own parts already encode in their z offsets, so one `renderOrder` for the whole surface is enough.
  - `On screen (flat)` (`overlay: 'screen'`). Painted onto the display at a fixed NDC rect: identical in both eyes, no depth, no parallax, fixed apparent size, and none of the panel's anchoring yaw. Same pixels in both eyes is something world space cannot express — one object has one transform and the eyes are 64 mm apart — so the placement runs per rendering camera: each mesh gets an `onBeforeRender` that rewrites its `matrixWorld` for whichever eye is about to draw it, which three.js then turns into `modelViewMatrix`. Two things that pass in flat and break in stereo: the handler must derive its local transform from the untouched `matrix` chain rather than from the `matrixWorld` it overwrites (three.js calls `onBeforeRender` once per eye with no matrix update in between, so anything else applies twice on the second eye); and it must read `camera.matrixWorldInverse` rather than call `camera.worldToLocal`, which calls `updateWorldMatrix` and would recompose an XR eye camera's pose out from under the headset.

World panels deliberately get neither (`overlay: 'none'`): a panel bolted to a wall *is* part of the room and should be occluded by whatever stands in front of it.

**`Draw` and `Eyes` are independent and compose**, which is the point of both being live rows — `One (this side)` puts each side's panel (and its pip) on that eye's three.js layer, and works with either draw mode, so *one eye, painted on the screen* is a setting you can select rather than a rewrite. Two things the layer path needs that are easy to miss: leaving XR has to put everything back on layer 0 unconditionally, or a one-eyed panel is drawn for nobody on a flat camera; and the layer has to be re-applied as parts appear, since a label's mesh doesn't exist until its font loads and would otherwise stay on layer 0, showing half a menu to the other eye. **In XR the gesture is a state, not a switch:** your hand being at your temple is what "open" means, and away is what "closed" means. The pip is the delay before the menu catches up with your hand, in whichever direction the two currently disagree — hold your hand up and it fills toward opening; take it away and it fills toward closing; bring it back part-way through and it simply empties and nothing happens. That last part is the whole reason for showing a bar rather than just acting. The side follows which side of your *head* the hand is on, not which hand it is, so reaching across works. Speed blocks arriving but not leaving: a hand flung past your ear is not a menu press, while putting your hand down quickly is still putting your hand down. **The driving hand's thumbstick belongs to the menu** while it is open (`data-menu-sticks`, read by locomotion the same way `data-menu-locked` is) — per hand, so the other stick keeps its usual job rather than freezing you in place. **Off a headset it is backtick** — next to Tab, which is the watch — plus a tappable corner hint that doubles as the mobile control. Whether it draws to one eye is a live question, so the visor's own first menu row toggles it: `Eyes: Both / One`, alongside `Scrim` and a `Curve` slider, all adjustable from inside the visor in a headset rather than from a rebuild.
